...
- Keep `bun run lint` green: the stylesheet-contract and design-system checks enforce the CSS escape-hatch rules and the declaration ratchet, and the palette check enforces every colour theme's shade completeness, its chrome ladders, and its contrast floors.
