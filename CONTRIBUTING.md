...
The current capture policy uses `examples/all-components.mdx` as a complete fixture: component captures are selected from the changed component owner's path, while global inputs and the final tip capture every owned region plus a full-document capture. A component capture may match multiple instances; each instance is captured at its own bounds. Keep the full-document target even when adding a new component target because it is the cross-component and layout safety net.
The style-history job uses the Playwright-pinned Chromium, fixed device scale and viewport settings, deterministic color/compositor flags, and the repository's bundled font set. Generated capture manifests and verification receipts record the browser build, platform, and font-set fingerprint. Only the configured `ci-runner` authority class enforces pixel verdicts; default local runs are advisory-only, and comparisons report rendering-environment mismatches explicitly.
CI persists successful verification receipts under `test-results/style-history-receipts`. Descendant commit-hash rewrites reuse only a contiguous prefix whose parent and commit trees, capture-policy and environment fingerprints, capture plan, visual contract, and tip safety-net scope still match; any mismatch forces re-verification. Verification reports all commit failures from one run together. A one-channel antialiasing tolerance is the final backstop; an adjacent two-channel excursion is absorbed only when anchored to one-channel drift, and every absorbed pixel is recorded in the evidence ledger.
