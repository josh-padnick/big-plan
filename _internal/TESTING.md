...
| 5. Mounted island test                   | Wiring between browser modules that never reference each other, where a broken link is silent rather than loud. Mount the real components with `// @vitest-environment happy-dom` and drive them through the runtime responses they read. Reach for it only when no lower rung can hold the two ends at once. | [`src/review/browser/inputs-surface.browser.test.ts`](src/review/browser/inputs-surface.browser.test.ts) mounts the review controller beside the Inputs panel and proves the panel catches up when the page applies a newer answers record.                                                                                                                                                           |
| 6. Playwright browser journey            | A critical reader flow, cross-feature composition, or regression that lower rungs cannot express. Use the complete rendered document and the repository fixture.                                                                                                                                              | [`test/commenting-runtime.spec.ts`](test/commenting-runtime.spec.ts) proves staged comments through the local review runtime. [`test/navigation.spec.ts`](test/navigation.spec.ts) proves rendered-plan navigation. [`test/components.spec.ts`](test/components.spec.ts) checks readable content and dormant controls with JavaScript disabled.                                                       |
...
- **New wiring between browser modules:** prefer a rung-1 test of the decision each module makes.
  Mount the real components only when the behavior at risk is that one module stops telling another something, which no test of either module alone can fail on.
