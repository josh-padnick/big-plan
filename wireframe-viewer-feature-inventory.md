...
promise, what main implements, and where the missing controls last lived.
...
`Wireframe` is the only figure family whose authoring guidance describes a canvas viewer that the product does not ship.
This inventory names each promised feature, the shipped state on main, and the existing implementation the follow-up work should adopt rather than reinvent.
It exists because the promise is written into guidance that agents read while authoring plans, so an author can reasonably compose a wireframe deck expecting reader controls that are not there.
...
Every promise below is authored in `src/components/wireframe/wireframe.guidance.md`.

| Promised feature                                                           | Guidance   | Shipped on main                                                                                |
| -------------------------------------------------------------------------- | ---------- | ---------------------------------------------------------------------------------------------- |
| Expanded mode entered fit-to-viewport and centered on both axes            | :141, :151 | Not implemented. `fit()` is width-only, never centers, and there is no expanded mode to enter. |
...
Wireframe's guidance promise is, feature for feature, what `FlowDiagram` already does.
...
Until ship 4 lands, `wireframe.guidance.md` describes reader controls the product does not have.
If the ships above are deferred, the honest alternative is to narrow the guidance to what Wireframe implements, so authors stop composing against a viewer that is not there.
