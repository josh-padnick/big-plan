...
`src/review/server.ts` owns request security, the route allow-list, dispatch, and the runtime lifecycle; `src/review/review-route-context.ts` owns the named state shared across handlers, and the sibling route modules own every route handler, grouped as session, review-state, agent-exchange, snapshot-diff, and assets.
