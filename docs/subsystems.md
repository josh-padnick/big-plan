...
- Whether an agent is attached and whether it has narrated lately are two questions, and one signal cannot answer both.
  ...
  `agentHoldsClaimedWork` in `src/review/shared/agent-status.ts` is the one definition of held work, and it is deliberately blind to the lease, because the quiet turn it answers for has by definition already let its lease lapse.
  It is an activity and queue input: it explains a silence, so it may withhold advice premised on nobody being there - the reconnect disclosure, and the blocked reading on a message queued behind a live turn - and it never reaches a connection surface.
