...
- Whether an agent is attached and whether it has narrated lately are two questions, and one signal cannot answer both.
  ...
  The recovery section itself is never gated - it is the only place the recovery prompt and connector command are rendered, so hiding it would leave a reviewer with no route back - and its copy, not its visibility, carries that safety.
