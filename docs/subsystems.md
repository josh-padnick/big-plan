...
- Whether an agent is attached and whether it has narrated lately are two questions, and one signal cannot answer both.
  ...
  It may not deny one either, so every gap in the signal - the health card, the connection log's summary, its rows and durations and counters, and the reason the runtime stores on the edge - names the missing signal and the quiet period it opens, never a disconnection and a reconnection.
  ...
  It is an activity and queue input: while it explains a silence it may withhold advice premised on nobody being there - the reconnect disclosure, and the blocked reading on a message queued behind a live turn - and it never reaches a connection surface.
  That explanation is bounded, because nothing reaps a claim and an unbounded one would withhold the reviewer's only route back to a working agent forever.
  Past `AGENT_RECOVERY_HORIZON_MS`, measured from the claim's own last signal and never from the lease, the pickup stops explaining anything everywhere at once: the stalled reading gives way to the ordinary presence answer, a newly sent message reads blocked rather than queued, and the recovery disclosure returns naming the takeover it now costs.
  The horizon is itself an inference from silence, which is the class of inference this rule otherwise removes; the takeover-aware wording is what keeps it honest, because it hands the adr/0002 consequence to the reviewer instead of nudging them into it.
