...
Components are flow-level JSX elements from the built-in [component registry](/components/): `Callout`, `CodeDiff`, `CodeSnippet`, `DatabaseTableSchema`, `FileTree`, `FileTreeDiff`, `GraphqlOperation`, `GrpcMethod`, and `HttpEndpoint`, plus scoped child components such as `Annotation` that are valid only inside a declaring parent.
