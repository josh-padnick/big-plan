...
- Every review also gets a stable local address that stays identical through runtime restarts, even when the replacement runtime's direct debugging address changes. The CLI presents this stable address as the one to open and labels the runtime address for debugging only. A small loopback service serves the live review while one runs, holds an unexpectedly interrupted review for its replacement, explains deliberate endings, starts itself when a command prints an address, and is inspected or stopped with `big-plan service`.
