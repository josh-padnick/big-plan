...
- The `Approval message` page holds the covering note a plan approval will carry once approving lands; approving itself is not here yet. One note covers every plan; there is no separate one per plan. It starts from a standard wording, accepts up to 2,000 characters, saves as you type, and `Reset to default` puts the standard wording back. Emptying the field does the same thing: a blank note falls back to the standard wording, so an approval never carries nothing. Like the other settings, it is saved for every review document in this browser.
