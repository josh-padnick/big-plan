...
- Confirmed decision answers are saved with the review and stay current only while their decision's content is unchanged; they remain in the review until approval, when the approval request delivers the recorded answers to the agent, and a standalone rendered document keeps answers for the reading session only.
