...
- **Approve plan** sits in the branding bar of a live, authoritative review. The dialog shows change sets, decisions, and the covering note from Settings; confirming writes a durable approval record, pins the current plan version, and stamps the page approved. A later source change offers Re-approve. Revoke returns the plan to review. Read-only sessions and static documents do not show the action.
