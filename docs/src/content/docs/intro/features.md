...
- Every review also gets a permanent local address that stays identical through runtime restarts, even when the replacement runtime's direct debugging address changes: it serves the live review while one runs, holds an unexpectedly interrupted review for its replacement, and explains deliberate endings. A small loopback service answers it, starts itself when a command prints an address, and is inspected or stopped with `big-plan service`.
