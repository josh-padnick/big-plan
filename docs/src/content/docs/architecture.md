...
When the renderer meets a typed block, the registry renders it to plain HTML on the server: the component's markup, styles, and behavior are baked into the output document, and no component code ships to the reader as something to execute.
...
It makes no external requests, works offline, and stays readable with JavaScript disabled; scripts only enhance navigation, theme switching, and copy controls.
