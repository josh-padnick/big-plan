...
import database_table_schemaLight from "../../../assets/components/database-table-schema-light.png";
import database_table_schemaDark from "../../../assets/components/database-table-schema-dark.png";
...
<ThemeImage
  light={database_table_schemaLight}
  dark={database_table_schemaDark}
  alt="One table schema with key badges, a foreign key, and column notes"
/>

he figure opens with a header band carrying the schema-qualified name and table note, above a `Column | Type | Constraints | Default | Comment` grid in which every column is exactly one equal-rhythm row.
...
The grid and every section are server-rendered in authored order, and a wide grid scrolls inside the figure instead of widening the page.
With the viewer script active, drag a column header to reorder it or focus the header and use <kbd>Left</kbd> / <kbd>Right</kbd>; keyboard moves are announced.
