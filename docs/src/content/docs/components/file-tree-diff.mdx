...
A live change tree - the combined view carries status-tinted names, comment hints, and the header change summary, the side-by-side view pairs the untouched Current tree with the fully annotated Planned tree and its Show diff switch, and clicking any directory row folds it:

<ThemeFrame
  light="/embeds/file-tree-diff-light.html"
  dark="/embeds/file-tree-diff-dark.html"
  title="The change tree with status-tinted names, comment hints, the header change summary, and the combined and side-by-side view toggle"
  height="26rem"
/>
