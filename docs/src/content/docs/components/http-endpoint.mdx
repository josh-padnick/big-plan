...
The Request body section states what the body is in one place: the media type up front, the `in="body"` Params as the payload's fields, then the fenced example demonstrating it; body Params without a `Request` still render the section.
...
With JavaScript enabled, a card with two or more sections folds them behind a tab bar - Path, Query, Headers, Body, Responses - with entry counts where applicable and an accent underline on the active tab.
Click a tab or use Left Arrow, Right Arrow, Home, and End to move between sections.
