...
| `in`       | enum: `path` `query` `header` `body`    | Yes      | The location badge; entries group in this order when rendered.  |
...
At most one `Request` documents the request body: an optional `contentType` attribute rendered as a chip, and exactly one fenced code block as the body.
