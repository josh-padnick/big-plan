...
Scoped child components are valid only inside their declaring parent: `Annotation` in `CodeDiff` and `CodeSnippet`; `Param`, `Request`, and `Response` in `HttpEndpoint`; `Argument`, `Returns`, `Operation`, `Variables`, and `Response` in `GraphqlOperation`; `Field`, `Error`, and `Proto` in `GrpcMethod`; every protocol card also accepts one closing `Review` checklist.
