...
| [DecisionSet](/components/decision-set/)                  | Review question-led decisions with lifecycle status, options, signed tradeoffs, recommendations, and outcomes.       |
...
Scoped child components are valid only inside their declaring parent: `Annotation` in `CodeDiff` and `CodeSnippet`; `Ddl` in `DatabaseTableSchema`; `Decision`, `Option`, `Pro`, and `Con` in `DecisionSet`; `Param`, `Request`, and `Response` in `HttpEndpoint`; `Argument`, `Field`, `Returns`, `Operation`, `Variables`, and `Response` in `GraphqlOperation`; `Field`, `Error`, `Example`, and `Proto` in `GrpcMethod`.
