...
`Example` repeats with an optional `label` and exactly one fence, rendered together under one labeled Example section - typically a `json` request payload beside a `text` stream trace, since a stream's behavior over time is what streaming-unfamiliar reviewers need to see.
