...
import snippetLight from "../../../assets/components/code-snippet-annotated-light.png";
import snippetDark from "../../../assets/components/code-snippet-annotated-dark.png";
...
<ThemeImage
  light={snippetLight}
  dark={snippetDark}
  alt="An annotated snippet with the file header, file-absolute gutter, tinted anchor lines, and their annotation cards"
/>
...
- `startLine` paired with an `Annotation` but no `showLineNumbers`: an annotation's line claim must stay verifiable without JavaScript, so a snippet that anchors a note to a file-absolute line keeps its gutter visible at rest. A `startLine` excerpt with no `Annotation` may still omit `showLineNumbers` and reveal its gutter only on maximize.
