...
`TopBar` puts its title at the start and loose children at the end. A `Group` written first leads, drawn before the title, which is how a phone bar carries the back control iOS puts there; every later child trails, so use `Group`s inside a `Row` for any other split.
The compiler blocks multiple `PageHeader`s on one screen, a pane or collection wrapped in a `Group`, equal flexible thirds on desktop, ambiguous record-detail selection, four or more outlined sibling panels, phone desktop-shell primitives, more than one filled work action per screen, and ambiguous Stepper progress.
