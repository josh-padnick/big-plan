...
The one-filled-action and one-page-header rules each count an overlay as its own layer, so a dialog may carry its own primary action and its own header while the page beneath keeps one of each too.
...
The compiler blocks a second `PageHeader` or a second filled work action within one layer, a pane or collection wrapped in a `Group`, equal flexible thirds on desktop, ambiguous record-detail selection, four or more outlined sibling panels, phone desktop-shell primitives, and ambiguous Stepper progress.
