...
| `Icon`             | Required `name` and `label`; optional `labelled` and `size="sm\|md\|lg"`. A mark, never a control. `label` always reaches assistive technology; `labelled` also draws those words beside the glyph. |
