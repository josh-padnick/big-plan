...
- **`npx` resolves the latest published version at run time.** A machine that runs `npx big-plan` picks up whatever the registry currently serves, so it gets security fixes immediately and it also has no pinning. Pin a version (`npx big-plan@0.0.1`) or install globally if your environment requires a fixed, reviewed version.
- **Releases are published with npm provenance, from CI only.** Publishing happens exclusively in the tagged-release GitHub Actions workflow, using npm Trusted Publishing over OIDC rather than a long-lived token. That workflow refuses to publish unless the tag equals the package version and points at a commit on `main`, and it runs the full lint, build, generated-file-drift, unit, and end-to-end suites first. Every release is published to the `next` dist-tag and smoke-tested from a clean environment before it is promoted, so `latest` has been exercised as a real install. The provenance attestation lets you verify a published tarball was built by that workflow from this repository:
