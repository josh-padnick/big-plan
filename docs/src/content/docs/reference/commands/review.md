...
| **This tab's review session is out of date**          | The runtime is answering but no longer recognises this tab, because it was restarted or its store re-created under the page  | Use **Reload**; the agent card and every send control stay inert with this reason until the page is reloaded                 |
