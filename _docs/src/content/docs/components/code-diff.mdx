...
- **[`CodeSnippet`](/components/code-snippet/)** — reach for it when you are showing existing code the reviewer should read, with no change to mark.
- A plain fenced block — reach for it when the code has no file identity: a command, a config sample, or output to expect.
