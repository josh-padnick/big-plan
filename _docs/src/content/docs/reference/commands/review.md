...
| **Reconnecting to this review session**               | A request timed out; the page cannot tell an idle expiry from a stopped runtime                                              | Keep the tab open while it retries automatically; **Reload now** is an optional fallback when no browser-only input is pending |
| **Reconnecting to the restarted review session**      | The runtime restarted with a new session id while retaining this review's token and store                                    | Keep the tab open while it reattaches automatically without discarding your reading position or browser-only input             |
