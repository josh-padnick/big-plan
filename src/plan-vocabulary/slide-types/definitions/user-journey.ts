...
export const USER_JOURNEY_SLIDE_TYPE = {
  ...
  guidance: [
    ...
    'Open the container with a user-summaries overview slide in the standard convention: a lead line counting the actors ("The user journeys cover three actors:"), one bullet per actor carrying only its bold name, one sub-bullet per journey written as a link whose text is the journey\'s bold slide number followed by its action phrase and whose target is the slug of that journey\'s h2, and a closing line opening "Together, they show"; the sub-bullets carry the count, so never label an actor with one, and title the slide with its claim rather than restating the container name.',
