// Extracts DatabaseTableSchema's single dbml-fence contract from component
...
/** Returns raw schema text only when the block holds exactly one dbml fence. */
export const schemaSource = ({
  ...
  if (
    ...
    !isElement(code) ||
    code.tagName !== "code" ||
    !languageClasses(code).includes("language-dbml") ||
