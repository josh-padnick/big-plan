...
- Write one `<Entry section="..." gist="..." />` per section, in document order; `section` repeats the catalog name for most typed slides, a user journey's ultra-concise `toc` form, or the h2 title for an untyped slide, and lint verifies the 1:1 match.
