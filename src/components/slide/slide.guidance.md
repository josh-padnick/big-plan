...
`Slide` is the guidance hook for a recurring plan role: place one self-closing marker directly above the heading it describes.
Most markers describe an h2; a grouped `user-journey` marker describes an h3 inside an h2 group.
...
3. When one fits, write `<Slide type="..." />` immediately above its heading and follow that type's guidance.
...
The heading supplies this plan's title: normally distinct from the name, written with concrete nouns and active verbs, without evaluative adjectives, superlatives, slogans, or abstract noun stacks.
