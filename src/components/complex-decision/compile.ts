// Compiles ComplexDecision's criteria-matrix grammar into a render-ready model
...
export type ComplexDecisionStatus = "open" | "decided" | "deferred";
export type ComplexDecisionTone = "good" | "bad" | "mixed" | "neutral";
export type ComplexDecisionReversibilityRating =
  "easy" | "somewhat-hard" | "hard";

const COMPLEX_DECISION_STATUSES: ReadonlyArray<ComplexDecisionStatus> = [
  "open",
  "decided",
  "deferred",
];

const COMPLEX_DECISION_TONES: ReadonlyArray<ComplexDecisionTone> = [
  "good",
  "bad",
  "mixed",
  "neutral",
...
const REVERSIBILITY_RATINGS: ReadonlyArray<ComplexDecisionReversibilityRating> =
  ["easy", "somewhat-hard", "hard"];
...
export type CompiledComplexDecisionCriterion = {
  readonly id: string;
  readonly title: string;
  readonly detail: ReadonlyArray<ElementContent>;
};

export type CompiledComplexDecisionScore = {
  readonly verdict: string;
  readonly tone: ComplexDecisionTone;
  readonly detail: ReadonlyArray<ElementContent>;
};

export type CompiledComplexDecisionOption = {
  readonly id: string;
  ...
  readonly scores: ReadonlyArray<CompiledComplexDecisionScore | undefined>;
};

export type CompiledComplexDecisionReversibility = {
  readonly rating: ComplexDecisionReversibilityRating;
  readonly detail: ReadonlyArray<ElementContent>;
};

export type CompiledComplexDecision = {
  readonly id: string;
  readonly question: string;
  readonly status: ComplexDecisionStatus;
  readonly context: ReadonlyArray<ElementContent>;
  readonly detail: ReadonlyArray<ElementContent>;
  readonly reversibility?: CompiledComplexDecisionReversibility;
  readonly criteria: ReadonlyArray<CompiledComplexDecisionCriterion>;
  readonly options: ReadonlyArray<CompiledComplexDecisionOption>;
  readonly chosenOption?: CompiledComplexDecisionOption;
};

const COMPLEX_DECISION_SCHEMA = {
  question: { kind: "string", required: true, nonEmpty: true },
  status: { kind: "enum", values: COMPLEX_DECISION_STATUSES },
...
const SCORE_SCHEMA = {
  ...
  tone: { kind: "enum", values: COMPLEX_DECISION_TONES },
} satisfies ComponentAttributeSchema;
...
type ScoreEntry = {
  ...
  readonly score: CompiledComplexDecisionScore;
};
...
const alignScores = ({
  ...
  readonly criteria: ReadonlyArray<CompiledComplexDecisionCriterion>;
  readonly diagnostics: DiagnosticCollector;
}): ReadonlyArray<CompiledComplexDecisionScore | undefined> => {
  const titles = new Set(criteria.map(({ title }) => title));
  const byCriterion = new Map<string, CompiledComplexDecisionScore>();
...
const validateDecisionOptions = ({
  ...
  if (entries.length < 2) {
    diagnostics.add({
      message: "ComplexDecision must contain at least two Options",
      position,
    });
  }
  ...
  for (const entry of entries) {
    ...
    if (titles.has(title)) {
      diagnostics.add({
        message: `Duplicate Option title "${title}" in ComplexDecision`,
        position: entry.child.position,
      });
    }
    titles.add(title);
  ...
  for (const duplicate of recommended.slice(1)) {
    diagnostics.add({
      message:
        "ComplexDecision cannot contain more than one recommended Option",
  ...
  for (const duplicate of chosen.slice(1)) {
    diagnostics.add({
      message: "ComplexDecision cannot contain more than one chosen Option",
      position: duplicate.child.position,
    });
  }
  if (status !== "decided") {
    for (const entry of chosen) {
      diagnostics.add({
        message:
          'A chosen Option requires its ComplexDecision to have status "decided"',
        position: entry.child.position,
      });
    }
    ...
    diagnostics.add({
      message:
        'A ComplexDecision with status "decided" must contain exactly one chosen Option',
      position,
    });
  }
};
...
const compileDetails = ({
  ...
  for (const duplicate of children.slice(1)) {
    diagnostics.add({
      message: "ComplexDecision cannot contain more than one Details",
...
const compileReversibility = ({
  ...
  readonly children: ReadonlyArray<ScopedChild>;
  readonly diagnostics: DiagnosticCollector;
}): CompiledComplexDecisionReversibility | undefined => {
  for (const duplicate of children.slice(1)) {
    diagnostics.add({
      message: "ComplexDecision cannot contain more than one Reversibility",
      position: duplicate.position,
    });
  }
  const child = children[0];
...
const validateCriteria = ({
  ...
  for (const child of children) {
    ...
    if (titles.has(title)) {
      diagnostics.add({
        message: `Duplicate Criterion title "${title}" in ComplexDecision`,
        position: child.position,
      });
    }
    titles.add(title);
...
/** Compiles one ComplexDecision component into the model consumed by rendering. */
export const compileComplexDecisionComponent = ({
  attributes,
  children,
  scopedChildren,
  position,
...
}: ComponentCompilerInput): CompiledComplexDecision => {
  const validated = validateComponentAttributes({
    component: "ComplexDecision",
    attributes,
    position,
    diagnostics,
    schema: COMPLEX_DECISION_SCHEMA,
  });
