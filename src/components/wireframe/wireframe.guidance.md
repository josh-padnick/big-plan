...
- **Content** - `Metric` for the number a screen exists to show, `Progress` for how far along something is, `Table` for rows of one kind of record, `List` and `ListItem` for simpler repeated rows, `Text`, `Heading`, `Badge`, `Divider`, and `ImagePlaceholder` for art nobody has drawn yet.
