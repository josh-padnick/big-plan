...
- Anchor a bar's two ends with two `Group`s inside `<Row justify="between">`. A `Row` distributes space between its children, so a title and three loose buttons each take a quarter of the bar; wrapping each side in a `Group` makes the row two items, and they settle where the product puts them. `TopBar` already reads this way - its title leads and its controls trail - so reach for `Group` when a `Row`, a panel header, or a command bar needs the same split.
