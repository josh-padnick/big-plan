...
- Size a mark standing alone by its job rather than by taste: `sm` rides a line of metadata, `md` stands with body copy, and `lg` is the one mark a screen is about or a target a finger reaches for. The steps are multiples of the artboard's own body type, so the same authored size is correct on every device.
- Never size a mark that stands with words. A mark drawn beside text - a `Button` carrying both an `icon` and its label, a `labelled` `Icon`, a `Reference` and the control that copies it - is contained to those words automatically: it is drawn a little above their cap height, centred on their line, one consistent step away. `size` picks a step on the standalone ramp only, so writing it on a `labelled` `Icon` is refused; if a mark beside words looks wrong, that is a defect in the containment rule rather than something to hand-tune per screen.
