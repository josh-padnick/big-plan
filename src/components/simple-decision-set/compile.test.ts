// Tests SimpleDecisionSet's pure compiler and end-to-end positional
...
const scoped = ({
  ...
  readonly name: "SimpleDecision" | "Option";
...
const compile = ({
  ...
  const model = compileSimpleDecisionSetComponent({
    attributes,
    children,
...
describe("compileSimpleDecisionSetComponent", () => {
  it("should compile the complete model when every layer is authored", () => {
    ...
    expect(model).toMatchObject({
      id: "simple-decision-set-open-questions",
      title: "Open questions",
      decisions: [
        {
          id: "simple-decision-set-open-questions-question-ship-now",
          question: "Ship now?",
          options: [
            {
              id: "simple-decision-set-open-questions-question-ship-now-option-yes",
              title: "Yes",
              recommended: true,
            },
            {
              id: "simple-decision-set-open-questions-question-ship-now-option-no",
              title: "No",
              recommended: false,
            },
          ],
        ...
        {
          id: "simple-decision-set-open-questions-question-rename-later",
  ...
  it("should deduplicate question ids when questions repeat", () => {
    ...
    expect(model.decisions.map(({ id }) => id)).toEqual([
      "simple-decision-set-question-same",
      "simple-decision-set-question-same-2",
    ]);
  });
    ...
    expect(diagnostics).toEqual([
      {
        line: 6,
        column: 1,
        message: 'Duplicate Option title "Same" in SimpleDecision',
      },
    ]);
  });
    ...
    expect(diagnostics).toEqual([
      {
        ...
        column: 1,
        message:
          "SimpleDecision cannot contain more than one recommended Option",
      },
    ]);
  });
});

describe("SimpleDecisionSet end-to-end diagnostics", () => {
  ...
  it("should leave SimpleDecision unknown outside its set", () => {
    expect(
      diagnosticsFor(
        '<SimpleDecision question="Q?">\n\n<Option title="A" />\n\n<Option title="B" />\n\n</SimpleDecision>\n',
      ),
    ).toEqual([
      {
        line: 1,
        column: 1,
        message: 'Unknown component "SimpleDecision"',
      },
      {
        line: 3,
        column: 1,
