...
describe("compile-time Mermaid renderer", () => {
  ...
  it("delivers a diagram in colour roles so it follows the reviewer's theme", () => {
    ...
    // Every literal a role owns is gone, in both variants: a colour left baked
    // in would freeze that part of the diagram in the palette that compiled it.
  ...
  it("returns a per-source failure instead of aborting the whole batch", () => {
    ...
    expect(failure !== undefined && isMermaidRenderFailure(failure)).toBe(true);
  ...
  it("maps every extracted node and edge to a Big Plan anchor", () => {
    ...
    const svg = rewriteMermaidSvgTargets({
      ...
      nodes: [
        ...
        { id: "b", label: "Beta", anchor: "component/MermaidDiagram#1/node/b" },
      ],
      edges: [
        {
          from: "a",
