...
The rendered HTML embeds all styling and branding assets, makes no external requests, and never executes plan-authored code; the only script is the shell's small embedded viewer script (TOC scroll-spy, hover popovers, and deck collapse), and every affordance keeps a no-JS fallback.
