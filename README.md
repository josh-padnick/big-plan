...
Rendered output follows the [two-artifact delivery contract](adr/0001-two-artifact-plan-delivery.md). The self-contained HTML embeds its styling and branding assets, makes no external requests, and never executes plan-authored code; its tiny head preference bootstrap and viewer scripts provide the [documented reader interactions](docs/src/content/docs/intro/features.md). The render stays readable with scripts disabled, while its interactive affordances require those embedded viewer scripts.
