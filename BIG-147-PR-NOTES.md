...
The takeover-aware wording is what keeps that honest: the returned disclosure says plainly that the first agent may still be running and finish on its own, that Big Plan cannot tell that apart from an agent that stopped, and that connecting a session takes the work over so the first agent's answer will no longer be accepted.
...
| 4. Agent process killed mid-turn             | Identical to case 1 for the first 30 minutes, and deliberately so: neither a slow agent nor a stopped one produces a signal, so claiming a disconnection would be the same unfounded assertion this bug is about. The claim being abandoned never makes a later session report a connection. Past the recovery horizon the pickup stops explaining the quiet: the card falls through to the ordinary presence answer, new messages read **Blocked - no agent connected**, and the Agent tab offers **Connect an agent and take over this work** with the takeover consequence stated. |
