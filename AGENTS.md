...
The static subset rejects imports, exports, expressions, and unsupported attributes with hard-fail diagnostics carrying line and column positions, while the built-in Callout, CodeDiff, CodeSnippet, DatabaseTableSchema, DecisionSet, FileTree, FileTreeDiff, GraphqlOperation, GrpcMethod, and HttpEndpoint components provide validated plan-native presentation.
...
DecisionSet renders question-led decision groups with lifecycle status, competing options, signed tradeoffs, recommendations, native long-form detail disclosures, and decided outcomes that keep losing options visible.
