...
- The plan model (`src/model/`) owns the framework-free contract consumed by render targets: compiled component models, shared attribute validation and diagnostics, and pure fence parsers. It imports no other project-local layer; `eslint.config.mjs` enforces that boundary.
- The React render target (`src/react/`) is the in-progress replacement for the per-component HAST renderers: every registry entry has a React component consuming compiled models from `src/model/` and rendering behind `render --renderer react`, with byte-parity against the vanilla output pinned by tests while both coexist. The registry retains its vanilla fallback until the React target becomes the default.
- The renderer (`src/render/`) is pure: MDX source plus a fallback title in, complete HTML or a validated plan model out. It uses unified (remark-parse, remark-gfm, remark-mdx, remark-rehype) to compile the static subset into a structured Hypertext Abstract Syntax Tree (HAST) review document, assigns heading slugs before the component registry so every generated id shares one namespace, then applies syntax highlighting, code-copy controls, and table scroll-container wrapping. Parsing and component validation collect positional diagnostics and hard-fail before the title, section outline, and element ids are collected, and rehype-stringify serializes HTML only after all transforms finish. The shell uses those ids to allocate a collision-free mobile `Overview` anchor.
