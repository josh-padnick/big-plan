...
- The UI library (`src/ui/`) owns the React components that consume compiled models from `src/model/` and render static HTML through each component contract's required `renderStatic` entry. React and react-dom run only during rendering; neither ships in exported documents.
- The renderer (`src/render/`) is pure: MDX source plus a fallback title in, complete HTML or a validated plan model out. It uses unified (remark-parse, remark-gfm, remark-mdx, remark-rehype) to compile the static subset into a structured Hypertext Abstract Syntax Tree (HAST) review document, assigns heading slugs before the component registry so every generated id shares one namespace, then applies syntax highlighting and table scroll-container wrapping. Parsing and component validation collect positional diagnostics and hard-fail before the title, section outline, and element ids are collected, and rehype-stringify serializes HTML only after all transforms finish. The shell uses those ids to allocate a collision-free mobile `Overview` anchor.
