...
DatabaseTableSchema renders one table's schema from a validated DBML-subset fence: a schema-qualified header, a psql-ordered columns grid with key badges and adjacent foreign-key and note lines, Indexes and Checks sections, and progressively enhanced table-name and raw-source copying.
