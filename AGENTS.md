...
- The React render target (`src/react/`) is the in-progress replacement for the per-component HAST renderers: one React component per ported registry entry, consuming compiled models from `src/model/` and rendering behind `render --renderer react`, with byte-parity against the vanilla output pinned by tests while both coexist.
