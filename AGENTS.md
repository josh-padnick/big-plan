...
DatabaseTableSchema renders one table's schema from a validated DBML-subset fence: a header band with the schema-qualified name and table note, a dense Column/Type/Constraints/Default grid whose Constraints cell carries key badges, nullability, foreign keys with uppercase SQL actions, and check expressions, an Indexes section with per-index invariant notes, and progressively enhanced table-name and raw-source copying.
