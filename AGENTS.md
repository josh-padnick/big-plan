...
DatabaseTableSchema renders one table's schema from a validated DBML-subset fence: a header band with the schema-qualified name and table note, a dense Column/Type/Constraints/Default/Comment grid keeping every column to one equal-rhythm row whose Constraints cell carries key badges, explicit nullability, foreign keys with uppercase SQL actions, and check expressions, a tinted name-first Indexes band with per-index invariant notes, and progressively enhanced table-name and raw-source copying.
