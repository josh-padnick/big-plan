...
BigDecision renders one weighty decision as a standalone card with lifecycle status, competing options, signed tradeoffs, a recommendation, native long-form detail disclosure, and a decided outcome that keeps losing options visible, while SmallDecisionSet collects a plan's small questions as a compact numbered list whose options pair a title with a short always-visible explanation, and undecided decisions in both components let the reader select an option locally as a preview of the choice a future live layer will transport.
