...
React is a presentation-edge implementation tool; no React runtime ships in a rendered document, and the only browser script is the shell's small self-contained viewer script for navigation scroll-spy, hover popovers, deck collapse, and wireframe navigation and scaling.
