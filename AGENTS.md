...
which scopes lookups to the live article, excludes inert snapshot copies inside legacy diff lenses, treats a compiler-addressed component diff replacement as live, and answers with an element or a reason it is missing.
...
| `src/review/`            | Local review persistence, loopback transport, agent exchange, staged plan mutation, causal snapshot diffs, the fixed-port service that answers saved review links under `service/`, and the browser interaction island. Keep browser-only React under `browser/`, browser-safe contracts under `shared/`, and Node-owned runtime behavior at the root. |
