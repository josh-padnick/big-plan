...
test("should diff a database schema at column level inside one rendering", async ({
  ...
  try {
    ...
    await test.step("refuse a text selection that crosses Was and Now", async () => {
      await sides.evaluateAll((sideNodes) => {
        const textNode = (node: Element): Text => {
          ...
          if (!(text instanceof Text)) throw new Error("side needs text");
          return text;
        };
        const range = document.createRange();
        range.setStart(textNode(sideNodes[0]), 0);
        const end = textNode(sideNodes[1]);
      ...
      await expect(
        page.locator("[data-selection-comment-tooltip]"),
      ).toHaveCount(0);
    });

    await test.step("bind a proposed-side text selection to Now", async () => {
      await sides.last().evaluate((side) => {
        const walker = document.createTreeWalker(
          side,
          NodeFilter.SHOW_TEXT,
          (candidate) =>
