...
// Quarantined as known-flaky under parallel load: the swapped article text can
// take longer than the assertion window to appear under contention (a
// swap-detection/morph timing issue, distinct from the post-morph scroll/
// selection restore this PR fixes), out of BIG-305 incident scope. Tracked by
// follow-up bp-big305-swap-timing-328; un-quarantine when it lands.
test.fixme("should review, reply to, and resolve a pushed thread in chat", async ({
  page,
}, testInfo) => {
  test.setTimeout(60_000);
