...
Mark a decision `critical` when the plan should not be approved until the reviewer answers it, and leave the rest unmarked: the review lists every question it is waiting on, and a plan where everything is critical tells the reviewer nothing about where to start.
